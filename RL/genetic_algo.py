...
def initialize_population(population_size, num_step):
    return [np.random.randint(1, 10, size=num_step) for _ in range(population_size)]
        ...
        for j in range(population_size):
            ...
            for k in range(len(individual)):
                ...
                if done == True:
                    ...
                    break
            if reward == 0:
                reward = 20  # If it does not reach the end of the curve after all steps, add penalty
            print(reward)
